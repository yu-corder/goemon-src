CC = gcc
CFLAGS = -Wall -Wextra -O2
TARGET_EXEC = kama-e
TARGET_COMP = kama-c

all: ${TARGET_EXEC} ${TARGET_COMP}

$(TARGET_EXEC): Kama/kama_execute.c
		$(CC) $(CFLAGS) -o $(TARGET_EXEC) Kama/kama_execute.c

$(TARGET_COMP): Kama/kama_compile.c Kama/scanner.c Kama/debug.c Kama/parser.c Kama/ast.c
		$(CC) $(CFLAGS) -o $(TARGET_COMP) Kama/kama_compile.c Kama/scanner.c Kama/debug.c Kama/parser.c Kama/ast.c

run: all
		./$(TARGET_COMP) --ast --token --binary examples/study.goe examples/study.gb
		./$(TARGET_EXEC) examples/study.gb

clean:
		rm -f $(TARGET_EXEC) $(TARGET_COMP) examples/*.gb

test: all
	./tests/run_tests.sh